...
struct circuit* line;
struct circuit* sort_Line;

struct list* list_Main;
struct list* list_Sec;
...
int main(int argc, char** argv){
  ...
  for(int i = 0; i < input_Var->num_Inputs; i++){
    add_Main(input_Var->inputs[i]);
  }
  add_Main("1");
  add_Main("0");
    
  while(sort_Size != line_Size){
    struct circuit* prev1 = NULL;

    for(struct circuit* ptr = line; ptr != NULL; ptr = ptr->next){
      if(!(strcmp(ptr->type, "MULTIPLEXER") == 0)){
	int isSort = 1;
	for(int i = 0; i < ptr->num_Input; i++){
	  if(check_List(ptr->inputs[i], list_Main) == 0){
	    isSort = 0;
	  }
	}
	...
	for(int i = 0; i < ptr->num_Para; i++){
	  if(check_List(ptr->para[i], list_Main) == 0){
	    isSort = 0;
	  }
	}
  ...
  if(list_Main != NULL){
    struct list* ptr1 = list_Main;
    while(ptr1 != NULL){
      struct list* temp = ptr1->next;
      //free(ptr1->data);
      free(ptr1);
      ptr1 = temp;
    }
  ...
  if(list_Sec != NULL){
    struct list* ptr2 = list_Sec;
    while(ptr2 != NULL){
      struct list* temp = ptr2->next;
      free(ptr2->data);
      free(ptr2);
      ptr2 = temp;
    }
...
void add_Sort(struct circuit* prev, struct circuit* ptr){
  struct circuit* temp = getLast();
  if(prev == NULL){
    if(temp == NULL){
      line = ptr->next;
      ptr->next = NULL;
...
int check_List(char* curr, struct list* front){
  for(struct list* ptr = front; ptr != NULL; ptr = ptr->next){
    //printf("%s", ptr->data);
    if(strcmp(curr, ptr->data) == 0){
      return 1;
    }
  }
  return 0;
...
void transfer(){
  for(struct list* ptr = list_Sec; ptr != NULL; ptr = ptr->next){
    add_Main(ptr->data);
  }
  
  struct list* ptr = list_Sec;
  while(ptr != NULL){
...
void add_Main(char* curr){
  if(list_Main == NULL){
    list_Main = malloc(sizeof(struct list));
    list_Main->data = curr;
    list_Main->next = NULL;
  }else{
...
void add_Sec(char* curr){
  if(list_Sec == NULL){
    list_Sec = malloc(sizeof(struct list));
    list_Sec->data = curr;
    list_Sec->next = NULL;
  }else{
    struct list* temp = malloc(sizeof(struct list));
